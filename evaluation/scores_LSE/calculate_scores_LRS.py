...
for videofile_idx in prog_bar:
	...
	prog_bar.set_description(
		f'Avg Confidence: {round(avg_confidence / (videofile_idx + 1), 3)}, Avg Minimum Dist: {round(avg_min_distance / (videofile_idx + 1), 3)}'
	)
	prog_bar.refresh()

print(f'Average Confidence: {avg_confidence / len(all_videos)}')
print(f'Average Minimum Distance: {avg_min_distance / len(all_videos)}')
